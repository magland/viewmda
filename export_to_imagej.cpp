...
bool export_to_imageJ(Mda &X, int d1,int d2,int d3,bool real_only) {
	...
	if (!QFile(imageJ_exec).exists()) {
		...
		if (!QFile(default_path).exists()) {
			default_path="c:\\progra~2\\imageJ\\imageJ.exe";
		}
		if (!QFile(default_path).exists()) {
			default_path="c:\\progra~3\\imageJ\\imageJ.exe";
		}
		#endif
		if (QFile(default_path).exists())
	...
	for (long l=0; l<N3; l++) {
		inds[d3]=l;
		float *buf=(float *)malloc(sizeof(float)*N1*N2);
		long cct=0;
		for (long k=0; k<N2; k++) {
			inds[d2]=k;
			for (long j=0; j<N1; j++) {
		...
		fwrite(buf,sizeof(float),N1*N2,outf);
		free(buf);
	}
	fclose(outf);
