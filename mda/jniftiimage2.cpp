...
public:
	...
	void copy_from(const JNiftiImage2 &X);
};
...
JNiftiImage2::JNiftiImage2() 
{
	d=new JNiftiImage2Private;
	d->q=this;
...
JNiftiImage2::JNiftiImage2(const JNiftiImage2 &X) {
	d=new JNiftiImage2Private;
	d->q=this;
	d->m_N1=d->m_N2=d->m_N3=d->m_N4=0;
	d->m_data_uchar=0;
...
void JNiftiImage2::operator=(const JNiftiImage2 &X) {
	d->copy_from(X);
}
...
QChar make_random_alphanumeric_niftiimage2() {
	static int val=0;
	val++;
...
QString make_random_id_niftiimage2(int char_count) {
	QString ret;
	for (int i=0; i<char_count; i++) {
		ret.append(make_random_alphanumeric_niftiimage2());
	}
	return ret;
}
...
bool JNiftiImage2::read(QString nii_fname,bool load_data) {
	#ifdef WIN32
	nii_fname.replace("/","\\");
	#endif
	...
	if (suf=="gz") {
		...
		return false;
		#endif
		QString tmp_fname=QDir::tempPath()+"/"+make_random_id_niftiimage2(10)+".nii";
...
bool JNiftiImage2::write(QString nii_fname) {
	bool ok=false;
	...
	for (int ii=0; ii<4; ii++) {
		img->sto_xyz.m[ii][jj]=d->m_transformation.matrixValue(ii,jj);
		img->sto_ijk.m[ii][jj]=T_inv.matrixValue(ii,jj);
	}
...
int JNiftiImage2::N1() const {
	return d->m_N1;
}
int JNiftiImage2::N2() const {
	return d->m_N2;
}
int JNiftiImage2::N3() const {
	return d->m_N3;
}
int JNiftiImage2::N4() const {
	return d->m_N4;
}
bool JNiftiImage2::allocate(QString data_type,int N1,int N2,int N3,int N4) {
	return d->internal_allocate(data_type,N1,N2,N3,N4);
}
AffineTransformation JNiftiImage2::worldTransformation() const {
	return d->m_transformation;
}
void JNiftiImage2::setWorldTransformation(const AffineTransformation &T) {
	d->m_transformation=T;
}
bool JNiftiImage2Private::internal_allocate(QString data_type,int N1,int N2,int N3,int N4) {
	internal_clear();
	long NN=N1*N2*N3*N4;
	if (!NN) {
...
bool JNiftiImage2Private::set_data(void *ddd) {
...
void JNiftiImage2Private::internal_clear() {
...
Array3D JNiftiImage2::dataXYZ(int t) const {
	Array3D ret;
	ret.allocate(d->m_N1,d->m_N2,d->m_N3);
	long factor=d->m_N1*d->m_N2*d->m_N3;
	long offset=factor*t;
	if (d->m_data_type=="uchar") {
		long ct=0;
		for (int zz=0; zz<d->m_N3; zz++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_uchar[offset+ct],xx,yy,zz);
			ct++;
		}
	}
	else if (d->m_data_type=="int16") {
		...
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_int16[offset+ct],xx,yy,zz);
			ct++;
		}
	}
	else if (d->m_data_type=="int32") {
		long ct=0;
		for (int zz=0; zz<d->m_N3; zz++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_int32[offset+ct],xx,yy,zz);
			ct++;
		}
	}
	else if (d->m_data_type=="int64") {
		long ct=0;
		for (int zz=0; zz<d->m_N3; zz++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_int64[offset+ct],xx,yy,zz);
			ct++;
		}
	}
	else if (d->m_data_type=="real32") {
		...
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_real32[offset+ct],xx,yy,zz);
			ct++;
		}
	}
	return ret;
}
Array3D JNiftiImage2::dataXYT(int z) const {
	Array3D ret;
	ret.allocate(d->m_N1,d->m_N2,d->m_N4);
	if (d->m_data_type=="uchar") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_uchar[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt],xx,yy,tt);
		}
	}
	else if (d->m_data_type=="int16") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_int16[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt],xx,yy,tt);
		}
	}
	else if (d->m_data_type=="int32") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_int32[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt],xx,yy,tt);
		}
	}
	else if (d->m_data_type=="int64") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_int64[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt],xx,yy,tt);
		}
	}
	else if (d->m_data_type=="real32") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			ret.setValue(d->m_data_real32[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt],xx,yy,tt);
		}
	}
	return ret;
}
void JNiftiImage2::setDataXYT(const Array3D &X,int z) {
	if (d->m_data_type=="uchar") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_uchar[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt]=X.value(xx,yy,tt);
		}
	}
	else if (d->m_data_type=="int16") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_int16[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt]=X.value(xx,yy,tt);
		}
	}
	else if (d->m_data_type=="int32") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_int32[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt]=X.value(xx,yy,tt);
		}
	}
	else if (d->m_data_type=="int64") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_int64[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt]=X.value(xx,yy,tt);
		}
	}
	else if (d->m_data_type=="real32") {
		for (int tt=0; tt<d->m_N4; tt++)
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_real32[xx+d->m_N1*yy+d->m_N1*d->m_N2*z+d->m_N1*d->m_N2*d->m_N3*tt]=X.value(xx,yy,tt);
		}
	}
}
void JNiftiImage2::setDataXYZ(const Array3D &X,int t) {
	long factor=d->m_N1*d->m_N2*d->m_N3;
	long offset=factor*t;
	if (d->m_data_type=="uchar") {
		long ct=0;
		...
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_uchar[offset+ct]=X.value(xx,yy,zz);
			ct++;
		}
	}
	else if (d->m_data_type=="int16") {
		...
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_int16[offset+ct]=X.value(xx,yy,zz);
			ct++;
		}
	}
	else if (d->m_data_type=="int32") {
		...
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_int32[offset+ct]=X.value(xx,yy,zz);
			ct++;
		}
	}
	else if (d->m_data_type=="int64") {
		...
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_int64[offset+ct]=X.value(xx,yy,zz);
			ct++;
		}
	}
	else if (d->m_data_type=="real32") {
		...
		for (int yy=0; yy<d->m_N2; yy++)
		for (int xx=0; xx<d->m_N1; xx++) {
			d->m_data_real32[offset+ct]=X.value(xx,yy,zz);
			ct++;
		}
	}
}
void JNiftiImage2::clear() {
	d->internal_clear();
}
QMap<QString,QVariant> JNiftiImage2::headerParameters() const {
	return d->m_header_parameters;
}
QString JNiftiImage2::dataType() const {
