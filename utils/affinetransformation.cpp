...
QList<float> AffineTransformation::matrix() const {
	QList<float> ret;
	for (int i=0; i<4; i++) 
	for (int j=0; j<4; j++) {
		ret << d->m_matrix[i][j];
...
void AffineTransformation::setMatrix(const QList<float> &X) {
	int ct=0;
	for (int i=0; i<4; i++) 
	for (int j=0; j<4; j++) {	
		d->m_matrix[i][j]=X.value(ct);
		ct++;
	}
}
...
void AffineTransformation::setMatrix(const JMatrix &M) {
	for (int i=0; i<4; i++)
	for (int j=0; j<4; j++) {
		d->m_matrix[i][j]=M.value(i,j);
	}
}
...
float AffineTransformation::matrixValue(int i,int j) const {
	return d->m_matrix[i][j];
}
void AffineTransformation::setMatrixValue(float val,int i,int j) {
	d->m_matrix[i][j]=val;
}
void AffineTransformation::writeMda(QString mda_fname) const {
	Array2D X(4,4);
	for (int i=0; i<4; i++)
	for (int j=0; j<4; j++) {
		X.setValue(matrixValue(i,j),i,j);
	}
	X.write(mda_fname);
}
void AffineTransformation::readMda(QString mda_fname) {
	Array2D X(mda_fname);
	if (X.width()<=1) {
		setIdentity(); 
		return;
	...
	for (int i=0; i<4; i++)
	for (int j=0; j<4; j++) {
		setMatrixValue(X.value(i,j),i,j);
	}
}
